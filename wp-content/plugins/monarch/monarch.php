...
class ET_Monarch {
	...
	function add_stats_record_db() {
		...
		$this->add_stats_row( $stats_data_array[ 'network' ], $stats_data_array[ 'post_id' ], $_SERVER[ 'REMOTE_ADDR' ], '', $stats_data_array[ 'action' ], $stats_data_array[ 'media_url' ] );
		die();
	}
	...
	*/
	...
	function display_meta_box( $post ) {
		...
		$selected_areas = get_post_meta( get_the_ID(), '_et_monarch_updated', true )
												...
												foreach( $options_array as $option) {
													...
													case 'text' :
														printf(
															...
															esc_textarea( $current_option_value ),
															esc_attr( $current_option_name ),
															( isset( $option['class'] )
																? sprintf( ' class="%1$s"', esc_attr( $option['class'] ) )
																: ''
	...
	*/
		...
		if ( isset( $monarch_options[ 'follow_widget_custom_colors' ] ) && true == $monarch_options[ 'follow_widget_custom_colors' ] ) {
			$custom_css .= sprintf( '%1$s%2$s%3$s%4$s',
				( isset( $monarch_options[ 'follow_widget_bg_color' ] ) && '' !== $monarch_options[ 'follow_widget_bg_color' ] )
					? sprintf( ' .et_monarch .widget_monarchwidget li, .et_monarch .widget_monarchwidget.et_social_circle li i { background-color: %1$s !important; }',
	...
	function generate_pinterest_picker(){
		...
		if ( $need_pinterest_modal ) {
			...
			$images_container = '<div class="et_social_pin_images">';
			preg_match_all( '/< *img[^>]*src *= *["\']?([^"\']*)/i', $content, $images_array );
			foreach ( $images_array[1] as $image_src ) {
				$share_link = $this->get_share_link( 'pinterest', $image_src );
				$images_container .= sprintf(
					'<div class="et_social_pin_image">
						<a href="%1$s" rel="nofollow" class="et_social_icon et_social_share" data-social_name="pinterest" data-post_id="%3$s" data-social_type="share">
							<img src="%2$s" width=250 height=200 />
	...
	function display_sidebar() {
		...
		if ( $this->check_applicability( $monarch_options[ 'sharing_sidebar_post_types' ], 'sidebar' ) ) {
			printf(
				'<div class="et_social_sidebar_networks et_social_%8$s et_social_%6$s et_social_sidebar_%1$s%2$s%3$s%7$s%9$s">
					%4$s
					%5$s
				</div>',
			...
			if ( true != $monarch_options[ 'sharing_sidebar_mobile' ] ) {
				printf(
					...
					<div class="et_social_mobile et_social_fadein">
						...
						<span class="et_social_close"></span>
						<div class="et_social_networks et_social_autowidth et_social_simple et_social_rounded et_social_left%2$s">
							%3$s
						</div>
					</div>
		...
		if ( $this->check_applicability( $monarch_options['sharing_flyin_post_types'], 'flyin' ) ) {
			printf(
				...
				'' !== $monarch_options[ 'sharing_flyin_title_text' ] || '' !== $monarch_options[ 'sharing_flyin_message_text' ]
					? sprintf( '<h3>%1$s</h3>', esc_html( $monarch_options[ 'sharing_flyin_title_text' ] ) )
					: '',
				'' !== $monarch_options[ 'sharing_flyin_message_text' ] ? sprintf( '<p>%1$s</p>', esc_html( $monarch_options[ 'sharing_flyin_message_text' ] ) ) : '',
		...
		if ( $this->check_applicability( $monarch_options['sharing_popup_post_types'], 'popup' ) ) {
			printf(
				...
				'' !== $monarch_options[ 'sharing_popup_title_text' ] || '' !== $monarch_options[ 'sharing_popup_message_text' ]
					? sprintf( '<h3>%1$s</h3>', esc_html( $monarch_options[ 'sharing_popup_title_text' ] ) )
					: '',
				'' !== $monarch_options[ 'sharing_popup_message_text' ]
					? sprintf( '<p>%1$s</p>', esc_html( $monarch_options[ 'sharing_popup_message_text' ] ) )
					: '',
	...
	public static function display_widget( $type = 'widget', $shortcode_atts = '' ) {
		...
		if ( isset( $monarch_options[ 'follow_networks_networks_sorting' ] ) && ! empty( $monarch_options[ 'follow_networks_networks_sorting' ] ) ) {
			...
			foreach ( $monarch_options[ 'follow_networks_networks_sorting' ][ 'class' ] as $icon ) {
				...
				$is_follows_cached = ET_Monarch::check_cached_counts( '', $icon, 'follow', $monarch_options[ 'follow_networks_use_api' ] );
